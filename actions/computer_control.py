...
from actions.screen_processor import capture_screen, vision_analyze
...
def _vision_verify(description: str, expected_state: str = "visible") -> bool:
    """Check if an element is in the expected state using VLM."""
    prompt = f"Is the UI element '{description}' currently {expected_state} on the screen? Reply with ONLY 'YES' or 'NO'."
    result = vision_analyze(prompt)
...
def computer_control(
    ...
    if not action:
        raise ValueError("No action specified for computer_control.")
    ...
    try:
            ...
            for attempt in range(max_retries):
                ...
                if not coords:
                    if attempt < max_retries - 1:
                        print(f"[ComputerControl] 🔄 '{desc}' not found, retrying ({attempt+1}/{max_retries})...")
                        time.sleep(1)
                        continue
                    raise RuntimeError(f"Element not found on screen: '{desc}'")
                ...
                # Vision Verify: Check if the element is STILL there (meaning click might have failed or didn't change state)
                # Or if the goal was to make it disappear.
                if _vision_verify(desc, "visible"):
                    print(f"[ComputerControl] ⚠️ Click on '{desc}' likely failed (still visible).")
                    if attempt < max_retries - 1:
                        # Try to analyze why it failed
                        reason = vision_analyze(f"I just clicked on '{desc}' at {coords}, but it is still visible. Why might the click have failed? (e.g. window not in focus, element blocked, requires double click)")
                        print(f"[ComputerControl] 🧠 VLM Analysis: {reason}")
                        
                        if "double click" in reason.lower():
                             _click(x=coords[0], y=coords[1], clicks=2)
                             time.sleep(1.0)
                             if not _vision_verify(desc, "visible"):
                                 return f"Double-clicked '{desc}' after retry. Success."
                        
                        time.sleep(1)
                        continue
                else:
                    return f"Clicked '{desc}' at {coords}. Verified state change."

            raise RuntimeError(f"Failed to interact with '{desc}' after {max_retries} attempts.")
        ...
        raise RuntimeError(f"computer_control '{action}' failed: {e}")

from typing import Optional, Any, Callable
from actions.base import Action, ActionRegistry

class ComputerControlAction(Action):
    @property
    def name(self) -> str:
    ...
    def description(self) -> str:
        return "Direct computer control: type, click, hotkeys, scroll, move mouse, screenshots, find elements on screen."

    @property
    def parameters_schema(self) -> dict:
        return {'type': 'OBJECT', 'properties': {'action': {'type': 'STRING', 'description': 'type | smart_type | click | double_click | right_click | hotkey | press | scroll | move | copy | paste | screenshot | wait | clear_field | focus_window | screen_find | screen_click | random_data | user_data'}, 'text': {'type': 'STRING', 'description': 'Text to type or paste'}, 'x': {'type': 'INTEGER', 'description': 'X coordinate'}, 'y': {'type': 'INTEGER', 'description': 'Y coordinate'}, 'keys': {'type': 'STRING', 'description': "Key combination e.g. 'ctrl+c'"}, 'key': {'type': 'STRING', 'description': "Single key e.g. 'enter'"}, 'direction': {'type': 'STRING', 'description': 'up | down | left | right'}, 'amount': {'type': 'INTEGER', 'description': 'Scroll amount (default: 3)'}, 'seconds': {'type': 'NUMBER', 'description': 'Seconds to wait'}, 'title': {'type': 'STRING', 'description': 'Window title for focus_window'}, 'description': {'type': 'STRING', 'description': 'Element description for screen_find/screen_click'}, 'type': {'type': 'STRING', 'description': 'Data type for random_data'}, 'field': {'type': 'STRING', 'description': 'Field for user_data: name|email|city'}, 'clear_first': {'type': 'BOOLEAN', 'description': 'Clear field before typing (default: true)'}, 'path': {'type': 'STRING', 'description': 'Save path for screenshot'}}, 'required': ['action']}
...
ActionRegistry.register(ComputerControlAction)
