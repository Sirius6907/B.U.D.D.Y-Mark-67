"""
browser_control.py  —  BUDDY Browser Action Module
====================================================
...
class _BrowserSession:
    ...
    Phase 5: CAPTCHA tespiti ve sesli uyarı entegre edildi.
    """

    def __init__(self, browser_name: str, player=None):
    ...
    async def _async_close(self):
        ...
        if self._context:
            try:
                # Session snapshotting: Save state before closing
                if not isinstance(self._context, BrowserContext): # Just a safety check
                     pass
                
                # If not a persistent context, we could save state manually here
                # but launch_persistent_context already saves to the profile dir.
    ...
    async def _start_captcha_monitor(self):
        """Phase 5: Async loop to detect CAPTCHAs and alert the user."""
        print(f"[Browser] 🔍 CAPTCHA monitor started for {self.browser_name}")
        while True:
            try:
                if self._page and not self._page.is_closed():
                    ...
                    if found and not self._captcha_detected:
                        self._captcha_detected = True
                        msg = "Sir, I have detected a CAPTCHA requirement. Please resolve it so I can continue."
                        print(f"[Browser] ⚠️  CAPTCHA DETECTED!")
                        if self.player:
                            self.player.write_log("SYS: CAPTCHA Detected!")
                            # Voice alert system
                            if hasattr(self.player, "speak"):
    ...
    async def _launch(self):
        ...
        if engine_name == "firefox":
            ...
            self._monitor_task = asyncio.create_task(self._start_captcha_monitor())
        ...
        # 2) BUDDY fallback profili
        buddy_profile = str(Path.home() / ".buddy_profiles" / self.browser_name)
        Path(buddy_profile).mkdir(parents=True, exist_ok=True)
        
        # Snapshotting: If we have a state file, we could potentially inject it, 
        # but launch_persistent_context handles this by using the folder.
    ...
    async def go_to(self, url: str) -> str:
        ...
        • On PlaywrightTimeout we check whether the URL actually changed;
          if it did, treat it as success so BUDDY won't retry endlessly.
...
class _SessionRegistry:
    ...
    def _get_or_create(self, browser_name: str, player=None) -> _BrowserSession:
        with self._lock:
            if browser_name not in self._sessions:
                sess = _BrowserSession(browser_name, player=player)
                sess.start()
                ...
                if player:
                    self._sessions[browser_name].player = player
            return self._sessions[browser_name]

    def get(self, browser_name: str | None = None, player=None) -> _BrowserSession:
        if not browser_name:
        ...
        sess = self._get_or_create(browser_name, player=player)
        self._active_browser = browser_name
        return sess

    def switch(self, browser_name: str, player=None) -> str:
        browser_name = _ALIASES.get(browser_name.lower().strip(), browser_name.lower().strip())
        self._get_or_create(browser_name, player=player)
        self._active_browser = browser_name
...
def browser_control(
    ...
    try:
        sess = _registry.get(browser, player=player)
    except Exception as e:
...
ActionRegistry.register(BrowserControlAction)
