...
class WebSearchAction(Action):
    ...
    def description(self) -> str:
        return "Searches the web for any information."

    @property
    def parameters_schema(self) -> dict:
        return {'type': 'OBJECT', 'properties': {'query': {'type': 'STRING', 'description': 'Search query'}, 'mode': {'type': 'STRING', 'description': 'search (default) or compare'}, 'items': {'type': 'ARRAY', 'items': {'type': 'STRING'}, 'description': 'Items to compare'}, 'aspect': {'type': 'STRING', 'description': 'price | specs | reviews'}}, 'required': ['query']}

    def execute(self, parameters: dict, player: Optional[Any] = None, speak: Optional[Callable] = None, **kwargs) -> str:
        return web_search(parameters=parameters, player=player, speak=speak)
