...
def _ask_for_url(prompt_text: str = "YouTube video URL:") -> str | None:
    try:
        ...
        url = simpledialog.askstring("B.U.D.D.Y", prompt_text, parent=root)
...
class YoutubeVideoAction(Action):
    @property
    def name(self) -> str:
    ...
    def parameters_schema(self) -> dict:
        return {'type': 'OBJECT', 'properties': {'action': {'type': 'STRING', 'description': 'play | summarize | get_info | trending (default: play)'}, 'query': {'type': 'STRING', 'description': 'Search query for play action'}, 'save': {'type': 'BOOLEAN', 'description': 'Save summary to Notepad (summarize only)'}, 'region': {'type': 'STRING', 'description': 'Country code for trending e.g. TR, US'}, 'url': {'type': 'STRING', 'description': 'Video URL for get_info action'}}, 'required': []}

    def execute(self, parameters: dict, player: Optional[Any] = None, speak: Optional[Callable] = None, **kwargs) -> str:
        return youtube_video(parameters=parameters, player=player, speak=speak)

ActionRegistry.register(YoutubeVideoAction)
