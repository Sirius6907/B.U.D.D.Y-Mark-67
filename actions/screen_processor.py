...
def compress_image(img_bytes: bytes, source_format: str = "PNG") -> tuple[bytes, str]:
    if not _PIL:
...
def capture_screen() -> tuple[bytes, str]:
    ...
    return compress_image(png, "PNG")


def vision_analyze(prompt: str, image_bytes: bytes = None, mime_type: str = "image/jpeg") -> str:
    """Quick, context-aware visual analysis using Gemini VLM."""
    ...
    try:
        ...
        print(f"[Vision] ❌ vision_analyze failed: {e}")
        return f"ERROR: {e}"
...
class ScreenProcessAction(Action):
    @property
    def name(self) -> str:
    ...
    def description(self) -> str:
        return "Captures and analyzes the screen or webcam image. MUST be called when user asks what is on screen, what you see, analyze my screen, look at camera, etc. You have NO visual ability without this tool. After calling this tool, stay SILENT — the vision module speaks directly."

    @property
    def parameters_schema(self) -> dict:
        return {'type': 'OBJECT', 'properties': {'angle': {'type': 'STRING', 'description': "'screen' to capture display, 'camera' for webcam. Default: 'screen'"}, 'text': {'type': 'STRING', 'description': 'The question or instruction about the captured image'}}, 'required': ['text']}
...
ActionRegistry.register(ScreenProcessAction)
