...
def computer_settings(
    ...
    try:
        ...
        raise RuntimeError(f"Action failed ({action}): {e}")

from typing import Optional, Any, Callable
from actions.base import Action, ActionRegistry

class ComputerSettingsAction(Action):
    @property
    def name(self) -> str:
    ...
    def description(self) -> str:
        return "Controls the computer: volume, brightness, window management, keyboard shortcuts, typing text on screen, closing apps, fullscreen, dark mode, WiFi, restart, shutdown, scrolling, tab management, zoom, screenshots, lock screen, refresh/reload page. Use for ANY single computer control command. NEVER route to agent_task."

    @property
    def parameters_schema(self) -> dict:
        return {'type': 'OBJECT', 'properties': {'action': {'type': 'STRING', 'description': 'The action to perform'}, 'description': {'type': 'STRING', 'description': 'Natural language description of what to do'}, 'value': {'type': 'STRING', 'description': 'Optional value: volume level, text to type, etc.'}}, 'required': []}
...
ActionRegistry.register(ComputerSettingsAction)
