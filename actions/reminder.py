...
try:
    ...
    notification.notify(title="B.U.D.D.Y Reminder", message=message, timeout=15)
    notified = True
except Exception:
    pass
...
if not notified:
    try:
        ...
        ToastNotifier().show_toast("B.U.D.D.Y Reminder", message, duration=15, threaded=False)
        notified = True
    except Exception:
        pass
...
if not notified:
    try:
        import subprocess
        script = 'display notification "{{}}" with title "B.U.D.D.Y Reminder"'.format(
            message.replace('"', '')
        )
...
"""
    ...
    script_body = f"""# Auto-generated by B.U.D.D.Y reminder — do not edit
...
def _schedule_windows(target_dt: datetime, task_name: str,
    ...
    xml_content = (
        ...
        '  <RegistrationInfo><Description>B.U.D.D.Y Reminder</Description></RegistrationInfo>\n'
...
class ReminderAction(Action):
    ...
    def description(self) -> str:
        return "Sets a timed reminder using Task Scheduler."

    @property
    def parameters_schema(self) -> dict:
        return {'type': 'OBJECT', 'properties': {'date': {'type': 'STRING', 'description': 'Date in YYYY-MM-DD format'}, 'time': {'type': 'STRING', 'description': 'Time in HH:MM format (24h)'}, 'message': {'type': 'STRING', 'description': 'Reminder message text'}}, 'required': ['date', 'time', 'message']}

    def execute(self, parameters: dict, player: Optional[Any] = None, speak: Optional[Callable] = None, **kwargs) -> str:
        return reminder(parameters=parameters, player=player, speak=speak)

ActionRegistry.register(ReminderAction)
